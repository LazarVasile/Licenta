...
export class AdminCreateMenuComponent implements OnInit {
  ...
  public index = this.categories.length;
  ...
  addMoreProducts(category) {
    ...
    console.log(this.counter_categories[category]);
  } 

  createMenu() {
    // const elements = Array.from(document.querySelectorAll("option").innerHtml);
    // console.log(elements);
    for (let i = 0; i < this.categories.length; i++)
      for (let j = 0; j < this.productsToAdd[this.categories[i]].length; j++)
        this.list_ids.push(this.productsToAdd[this.categories[i]][j])
      
    let data = {"listIds" : this.list_ids, "dateMenu" : this.date_menu}
    console.log(data)
